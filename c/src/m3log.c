...
int m3log_format(const m3log_entry_t *entry, char *buffer, size_t buffer_size) {
  ...
  if (entry->time && strlen(entry->time) > 0) {
    written +=
        snprintf(buffer + written, buffer_size - written, "@%s ", entry->time);
  } else {
    char *timestamp = m3log_generate_timestamp();
    written +=
        snprintf(buffer + written, buffer_size - written, "@%s ", timestamp);
    free(timestamp);
  }
  ...
  for (size_t i = 0; i < entry->tags.count; i++) {
    written +=
        snprintf(buffer + written, buffer_size - written, "%s%s",
                 entry->tags.tags[i], (i < entry->tags.count - 1) ? " " : "");
  ...
  if (entry->level != M3LOG_LEVEL_UNKNOWN) {
    written += snprintf(buffer + written, buffer_size - written,
                        "#%s: ", m3log_level_to_string(entry->level));
  } else {
    written += snprintf(buffer + written, buffer_size - written, ": ");
  }
  ...
  if (entry->content) {
    written +=
        snprintf(buffer + written, buffer_size - written, "%s", entry->content);
  }
...
m3log_error_t m3log_log(m3log_level_t level, const char *tags,
                        const char *format, ...) {
  if (!format) {
    return M3LOG_ERROR_INVALID_ARGUMENT;
  }
  ...
  /* 创建日志条目 */
  m3log_entry_t *entry =
      m3log_create_entry(content, level, (const char **)tag_array, tag_count);
